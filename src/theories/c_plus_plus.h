...
namespace theory{
    ...
    public:
        ...
        static bool check_if_constraints(const Program *p, size_t program_line, const instructions::Instruction *new_ins){
            ...
            if(ins_if){
                ...
                for(size_t for_orig_line = 0; for_orig_line < dest_line; for_orig_line++){
                    auto for_ins = dynamic_cast<const instructions::For*>(p->get_instruction(for_orig_line));
                ...
                for (size_t prev_if_orig_line = 0; prev_if_orig_line < dest_line; prev_if_orig_line++) {
                    auto ins_prev_if = dynamic_cast<const instructions::If*>(p->get_instruction(prev_if_orig_line));
                ...
                if(dynamic_cast<const instructions::If*>(p->get_instruction(dest_line))) return false;
        ...
        static bool check_conditional_constraints(const Program *p, size_t program_line){
            ...
            for(size_t line = 0; line < program_line; line++){
                ...
                auto for_ins = dynamic_cast<const instructions::For*>(p->get_instruction(line));
                ...
                auto if_ins = dynamic_cast<const instructions::If*>(p->get_instruction(line));
        ...
        static bool check_pointer_action_constraints(const Program *p, size_t program_line, const instructions::Instruction *new_ins){
        ...
        static bool check_for_pointer_constraints(const  variables::Pointer* ptr1, const variables::Pointer* ptr2){
        ...
        static bool check_for_constraints(const Program *p, size_t program_line, const instructions::For *for_ins){
            ...
            for(size_t line = 0; line < dest_line; line++){
                auto prev_for_ins = dynamic_cast<const instructions::For*>(p->get_instruction(line));
            ...
            for(size_t line = program_line+1; line < dest_line; line++){
                auto pa = dynamic_cast<const instructions::PointerAction*>(p->get_instruction(line));
