...
namespace expressions {
    ...
    public:
        ...
        explicit Expression(std::string op = "",
                ...
                _operator_name(std::move(op)), _lhs(std::move(lhs)), _rhs(std::move(rhs)) {}
        ...
        [[nodiscard]] virtual std::unique_ptr<Expression> copy_expression() const {
        ...
        [[nodiscard]] value_t get_value(const variables::Variable* v, const State *s) const{
            auto state_variable = dynamic_cast<const variables::StateVariable*>(v);
            if(state_variable) return s->get_value(state_variable);
            auto constant_value = dynamic_cast<const variables::ConstantValue*>(v);
            if(constant_value) return constant_value->get_value();
            // add new variable types here
            return value_t{0}; // default value
        }

        void update_object_references(const std::vector<const Object*> &new_obj_refs){
            if(_lhs) _lhs->update_object_references(new_obj_refs);
            if(_rhs) _rhs->update_object_references(new_obj_refs);
        }

        void set_pointer_references(const std::vector<const variables::Variable*> &new_ptr_refs){
            if(_lhs) _lhs->set_pointer_references(new_ptr_refs);
            if(_rhs) _rhs->set_pointer_references(new_ptr_refs);
        }

        [[nodiscard]] const variables::Variable* get_lhs() const{
            return _lhs.get();
        }

        [[nodiscard]] const variables::Variable* get_rhs() const{
            return _rhs.get();
        }
    ...
    protected:
        ...
        const std::unique_ptr<variables::Variable> _lhs;
        const std::unique_ptr<variables::Variable> _rhs; // FIXME (Issue #48): rhs could be an Expression
    };
}
#endif
