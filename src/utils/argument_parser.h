    ...
    public:
        ...
        static void helper(const std::string &error_message = "", int error_code = ERROR_INPUT) {
            std::string help_str =
                    ...
                    " default: false\n"
                    "  " + _progressive_stype + ", " + _progressive_type + " PROGRESSIVE  available in \"synthesis\" or \"repair\" mode;"
                    " activates input instances progressively, where tentative solutions fails; "
                    " type: boolean (True, False; true, false; 1, 0);"
                    " default: false\n"
                    "  " + _threads_stype + ", " + _threads_type + " THREADS  available in \"synthesis\" or \"repair\" mode; "
                    " specifies the number of threads to use for the search engine; "
                    " type: strictly positive integer; "
                    " default: 1\n"
                    "  " + _parallel_strategy_stype + ", " + _parallel_strategy_type +
                    " {independent_queues, distribute_promising}   available in \"synthesis\" or \"repair\" mode with THREADS > 1; "
                    " specifies the parallelization strategy to use for the search engine; "
                    " default: distribute_promising\n"
                    "      independent_queues: threads work independently and do not share nodes\n"
                    "      distribute_promising: threads can send nodes to other threads if they have a promising heuristic value\n"
