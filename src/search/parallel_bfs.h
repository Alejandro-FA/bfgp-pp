...
namespace search {
    ...
    public:
        explicit ParallelBFS(BFSFactory auto &&f, RootsFactory auto &&g, unsigned int num_threads, unsigned int num_init_nodes = 8)
    ...
    private:
        ...
        const unsigned int _num_init_nodes{8}; // (minimum) number of initial search nodes per thread
