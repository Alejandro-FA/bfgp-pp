...
namespace search {
    ...
    public:
        ...
        /// Stop source used to interrupt the search when a solution is found. If the stop source is shared by multiple
        /// threads, it can be used to interrupt them.
        /// Careful! This method is public (i.e., it could be called from multiple threads) and not thread-safe.
        /// This method is intended to be called only once, before the search starts.
        void set_stop_source(std::stop_source ssource) {
            _stop_source = std::move(ssource);
        }

        /// Sets a limit to the _open queue size. If this limit is reached, the search will stop. Useful to create an
        /// initial amount of work to be distributed.
        /// Careful! This method is public (i.e., it could be called from multiple threads) and not thread-safe.
        ...
        /// Returns true if the frontier is empty. Parallel BFS implementations should use a thread-safe queue to avoid
        /// data races.
        [[nodiscard]] bool is_empty() const {
            return _open->empty();
        }

        /// Adds a new node to the search queue. Parallel BFS implementations should use a thread-safe queue to avoid
        /// data races.
        ...
        /// Parallel BFS implementations should protect calls to this method with a mutex or other synchronization
        /// mechanisms for thread-safety.
        /// Furthermore, parallel implementations should use a thread-safe queue to avoid data races.
        [[nodiscard]] virtual std::shared_ptr<Node> select_node() {
            if (_reevaluation_required) { // First, ensure that queue evaluations are up-to-date
                _open->reevaluate([this](const Node* node) { return f(node); }, _gpp.get());
                if (_verbose) std::osyncstream{std::cout} << "[ENGINE " << _id << "] Reevaluation done!\n";
                _reevaluation_required = false;
            }
            return _open->select_node(); // Then, return the next best node
        }
        ...
        /// Parallel BFS implementations should protect calls to this method with a mutex or other synchronization
        /// mechanisms for thread-safety.
        [[nodiscard]] virtual const vec_value_t& current_evaluations() const {
            return _current_evaluations;
        }

        /// Careful! This method is public (i.e., it could be called from multiple threads) and not thread-safe. In a
        /// multithreaded environment, this method should only be called by a single thread at a time.
...
//std::cout << "[INFO] Total new expansions = " << children.size() << "\n";
                ...
                for (const auto& child: expand_node(current.get())) {
    ...
    protected:
        ...
        /// nodes to other threads instead of adding them to the queue directly.
        ...
        /// this method to synchronize the activation between threads.
        virtual void activate_instance_request(id_type instance_idx) {
            ...
            activate_instance(instance_idx);
        }
    ...
    private:
        ...
        bool _reevaluation_required {false};
    };
}
