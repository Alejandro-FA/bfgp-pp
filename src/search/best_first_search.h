...
//std::cout << "[INFO] checking new expansion with instruction:\n" << ins->to_string(true) << "\n";
        ...
        [[nodiscard]] std::shared_ptr<Node> solve(std::vector<std::unique_ptr<Program>> roots = {}) override {
            ...
            if(roots.empty()){
                ...
                std::shared_lock lock{_pgp_mutex}; // Ensures that during evaluation no new instances are activated
...
//std::cout << "[INFO] New child to evaluate:\n" << child->to_string() << "\n";
                    ...
                    { // The lock of this section prevents other threads from activating instances, but allows them read access
                        std::shared_lock lock{_pgp_mutex};
                    ...
                    if (activate_instance) {
                        if (_verbose) std::cout << "[INFO] Failure on instance " + std::to_string(_last_failed_instance_idx + 1) + " reevaluating... ";
                        activate_instance_request(_last_failed_instance_idx); // Activate the failed instance and reevaluate the queue.
        ...
        void activate_and_reevaluate(id_type instance_idx) {
            // TODO: Understand why the reevaluation needs to be placed inside the unique lock and cannot be placed under a shared lock
            //  The original idea was to activate in unique lock, and reevaluate in shared lock. This leads to data races.
    ...
    private:
        ...
        mutable std::shared_mutex _pgp_mutex; // Mutex to protect the active instances of a GPP from being modified while a node is being evaluated and added to the queue
