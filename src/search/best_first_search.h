...
namespace search {
    ...
    public:
        ...
        /// NOTE: This method can be called from different threads, so it must be thread-safe. In multithreaded
        /// environments, you might need to use a ThreadSafeFrontier.
        ...
        /// NOTE: This method can be called from different threads, so it must be thread-safe. In multithreaded
        /// environments, you might need to use a ThreadSafeFrontier. This method does not check if the frontier is
        /// empty. In a multithreaded execution, reevaluating the queue here does not guarantee that externally added
        /// nodes have been evaluated with the same instances. However, the synchronization overhead required for
        /// achieving full consistency is not worth it.
        [[nodiscard]] std::shared_ptr<Node> select_node() {
            // First we activate failed instances if needed
            bool reevaluate = false;
            if (_gpp->is_progressive()) {
                std::scoped_lock lock{_pgp_mutex};
                for (const auto& instance_idx : _instances_to_activate) {
                    if (not _gpp->is_instance_active(instance_idx)) {
            ...
            // If there were instances to activate, reevaluate the queue
            if (reevaluate) {
            ...
            // Finally, return the next best node
        ...
        void add_instance_to_activate(id_type instance_idx) {
            std::scoped_lock lock{_pgp_mutex};
...
//std::cout << "[INFO] New child to evaluate:\n" << child->to_string() << "\n";
    ...
    protected:
        ...
        /// this method to share the activation.
        virtual void activate_instance_request(id_type instance_idx) {
            ...
            add_instance_to_activate(instance_idx);
        }
