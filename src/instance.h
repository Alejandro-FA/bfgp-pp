...
class Instance{
public:
	explicit Instance(std::string name = "", id_type instance_id = 0, const Domain *dom = nullptr ) :
            _domain(dom), _name(std::move(name)), _init(nullptr), _goal_state(std::make_unique<State>()), _instance_id(instance_id){
    ...
    void add_object(std::unique_ptr<const Object> obj){
	...
	void add_goal_condition(std::unique_ptr<expressions::conditions::Condition> cond ){
        ...
        auto lhs_sv = dynamic_cast<const variables::StateVariable*>(cond->get_lhs());
        auto rhs_cv = dynamic_cast<const variables::ConstantValue*>(cond->get_rhs());
    ...
    // Const overload
    [[nodiscard]] State* get_initial_state(){
        return _init.get();
    }

	[[nodiscard]] const State* get_initial_state() const{
		return _init.get();
	}

    // ToDo: inefficient, is there a faster way to return raw pointers from a vector of unique_ptr?
    [[nodiscard]] std::vector<const expressions::conditions::Condition*> get_goal_condition() const{
        std::vector<const expressions::conditions::Condition*> goal_conds;
	...
	[[nodiscard]] const expressions::conditions::Condition* get_condition(id_type idx ) const{
        assert(idx < (id_type)_goal.size());
		return _goal[ idx ].get();
	}

    // FIXME (Issue #47): inefficient method
    [[nodiscard]] std::vector<const Object*> get_raw_objects() const{
        std::vector<const Object*> raw_obj_ptrs;
        for(const auto& obj : _objects)
            raw_obj_ptrs.emplace_back(obj.get());
        return raw_obj_ptrs;
    }

    [[nodiscard]] size_t get_typed_object_idx(const Object *obj) const{
        auto it = _typed_obj_to_idx.find(obj->get_name());
        assert(it != _typed_obj_to_idx.end());
        return it->second;
    }

    [[nodiscard]] const Object* get_object(size_t idx) const{
        /// Get a raw object pointer by its index
        assert(idx < _objects.size());
        return _objects[idx].get();
    }

    [[nodiscard]] const Object* get_object(const std::string& name) const{
    ...
    [[nodiscard]] std::vector<const Object*> get_typed_objects(const std::string& name) const{
    ...
    [[nodiscard]] const State* get_goal_state() const{
